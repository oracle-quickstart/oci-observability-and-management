...
#    current_time = local.current_time
  ...
  provisioner "local-exec" {
    when = destroy
    command = "python3 ./scripts/manageassocs.py -o delete -a ${self.triggers.auth_type} -p ${self.triggers.profile_name} -c ${self.triggers.compartment_id} -e ${self.triggers.entity_id} -l ${self.triggers.loggroup_id} -f ${self.triggers.path}"
  }
}
